...
public class LegacyUserStorageProvider
  ...
  @SuppressFBWarnings(
      ...
      justification =
          "Keycloak session and component model lifecycles are managed by the Keycloak runtime")
  public LegacyUserStorageProvider(KeycloakSession session, ComponentModel model) {
    this.session = session;
    this.model = model;
    String baseUrl =
        Optional.ofNullable(model.get(CONFIG_BASE_URL)).orElse("http://legacy-auth:4000/");
  ...
  public boolean isValid(RealmModel realm, UserModel user, CredentialInput credentialInput) {
    ...
    boolean valid =
        legacyUserService.validateCredentials(username, credentialInput.getChallengeResponse());
    if (!valid) {
      return false;
    }
  ...
  private Optional<LegacyUserRepresentation> loadUser(String username) {
    ...
    fetched
        .map(LegacyUserStorageProvider::copyRepresentation)
  ...
  private static LegacyUserRepresentation copyRepresentation(
      LegacyUserRepresentation representation) {
    return new LegacyUserRepresentation(representation);
  }

  private void importUserIfNeeded(
      RealmModel realm, LegacyUserRepresentation representation, String password) {
